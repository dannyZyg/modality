...
Scale::Scale(const std::string& name) {
    ...
    auto it = std::find_if(scaleDefinitions.begin(), scaleDefinitions.end(),
        [&name](const auto& def) { return def.first == name; });
...
Degree Scale::getHigher(const Degree& d) {
    ...
    if (d.value >= 0.0) {
        ...
        auto it = std::find_if(descendingDegrees.begin(), descendingDegrees.end(),
            [d](double val) { return std::abs(val - d.value) < 0.0001; });
...
std::vector<double> Scale::getDescendingDegrees() const {
    ...
    if (degrees.empty()) return descending;
